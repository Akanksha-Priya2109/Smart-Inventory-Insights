...
class Register:
    def __init__(self,root):
        ...
        #Bg Imagef
        self.bg=ImageTk.PhotoImage(file="images/back2.jpg")
        bg = Label(self.root,image=self.bg).place(x=250,y=0,relwidth=1, relheight=1)
        
        #LEFT Image===
        self.left=ImageTk.PhotoImage(file="images/sides.png")
        ...
        self.var_chk = IntVar()
        chk = Checkbutton (frame1, text="I Agree The Teims & Conditions", variable=self.var_chk, onvalue=1,offvalue=0,bg="white",font=("times new roman",12)).place(x=50,y=380)
...
#______________________________________________________________________FUNCTION____________________________________
    ...
    def register_data(self):
        if self.txt_fname.get()=="" or  self.txt_contact.get()=="" or self.txt_email.get()=="" or self.cmb_quest.get()=="Select" or self.txt_answer.get()=="" or  self.txt_password.get() =="" or self.txt_cpassword.get()=="":
            ...
            try:
                ...
                cur=con.cursor()
                cur.execute("select * from administrator where email=?",(self.txt_email.get(),))
                row=cur.fetchone()
                if row!=None:
                    messagebox.showerror("Error", "User already Exist,Please try with another email", parent=self.root)
                else:
                    cur.execute("insert into administrator (f_name,l_name,contact,email,question,answer,password) values(?,?,?,?,?,?,?)",
                                (self.txt_fname.get(),
                                self.txt_lname.get(),
